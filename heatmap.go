...
// Maps all values in the input map to the interval [0,1] by dividing each value
// by the maximum value in the input
// Note: Assumes that entries are non-negative
func max_normalize(m map[Point]float64) (n map[Point]float64) {

	if len(m) < 1 {
		panic(fmt.Sprintf("Attempted to normalize an empty map %v", m))
	} 
...
// Normalize by dividing each value by the sum of the values in the input
func tot_normalize(m map[Point]float64) (n map[Point]float64) {
...
func heatmap(t int, record [][]NodeRecord) map[Point]float64 {
	return tot_normalize(floatify(sum(counts(t, record))))
}
...
func weighted_heatmap(t int, record [][]NodeRecord) map[Point]float64 {
	return max_normalize(weighted_sum(counts(t, record)))
}
